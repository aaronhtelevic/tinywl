...
bool swipe = false;
double swipe_pos_x = 0;
double swipe_pos_y = 0;

static void handle_touch_event_down(struct wl_listener *listener, void *data) {
	struct tinywl_touch *touch = wl_container_of(listener, touch, down);
	struct tinywl_server *server = touch->server;
...
static void handle_touch_event_up(struct wl_listener *listener, void *data) {
	struct tinywl_touch *touch = wl_container_of(listener, touch, up);
	struct tinywl_server *server = touch->server;
	struct wlr_touch_down_event *event = data;
...
static void handle_touch_event_motion(struct wl_listener *listener, void *data) {
	struct tinywl_touch *touch = wl_container_of(listener, touch, motion);
	struct tinywl_server *server = touch->server;
	struct wlr_touch_motion_event *event = data;

	fprintf(stderr, "MOTION event_x: %f, event_y: %f\n", event->x, event->y);
	if (swipe) {
		// update the swipe position
		swipe_pos_x = event->x;
		swipe_pos_y = event->y;
	}
}

static void server_new_touch(struct tinywl_server *server,
		struct wlr_input_device *device) {
	// we should do something here
	struct wlr_touch *wlr_touch = wlr_touch_from_input_device(device);

	struct tinywl_touch *touch = calloc(1, sizeof(*touch));
	touch->server = server;
	touch->wlr_touch = wlr_touch;

	touch->down.notify = handle_touch_event_down;
	wl_signal_add(&wlr_touch->events.down, &touch->down);
	touch->up.notify = handle_touch_event_up;
	wl_signal_add(&wlr_touch->events.up, &touch->up);

	touch->motion.notify = handle_touch_event_motion;
	wl_signal_add(&wlr_touch->events.motion, &touch->motion);
}
